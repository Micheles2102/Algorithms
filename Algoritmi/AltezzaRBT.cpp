...
{
previous = pointer;
if(Key <= pointer->getKey()) pointer = pointer->getLeft();
...
{
if(Key <= previous->getKey()) previous->setLeft(node);
else previous->setRight(node);
}
...
{
if(node != NilNode) return max(_height(node->getLeft()) + 1,_height(node->getRight()) + 1);
return 0 ;
}
